...
class GPTNeoXAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if hasattr(self, "loc_list") and self.loc_list is not None:
            # print("do mask")
            causal_mask = causal_mask.repeat(attn_scores.shape[0],1,1,1)
            for batch_idx, [attend_area_begin, attend_area_end], [start, end] in self.loc_list:
                causal_mask[batch_idx, 0, start:end+1, :attend_area_begin] = False
...
class GPTNeoXForCausalLM(GPTNeoXPreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self, "visual_guided_lm_head"):
            lm_logits, attention_maps = self.visual_guided_lm_head(
                lm_logits, hidden_states, input_ids, self.image_embedding,
                self.previsual_token_id, self.visual_token_id, self.media_token_id,
            )
